classdef Robot < handle
    ...
    methods
        ...
        function T = calculate_transform2(self, c)
            % ********************************************************
            %                          TODO
            % ********************************************************
            T = eye(4);
            for ii = 1 : size(self.lls, 2)
                jj = 1;
                Tii = self.arckinematics(c(jj), c(jj+1), c(jj+2));
                T = T * Tii;
                jj = jj+3;
            end
        ...
        function [rho_des, theta_des] = inv_kin_dep(self, ind_params_des, q_current)
            ...
            for iter = 1:maxIter
                ...
                J_dep = self.central_diff_jacobian(f_handle, q, 1e-7);
    ...
    function ind_params = inv_kin_ind(self, traj, q_current)
        ...
        for i = 1:n_segs
            ...
            c_current(idx+2) = self.lls(i);
        end
        c_current = c_current' %needs to be a column vector
        maxIter = 1000;
        tol     = 1e-3;
        lambda  = 0.1;
        ...
        for kk = 1:nPts
            ...
            for iter = 1:maxIter
                % Forward kinematics
                T = self.calculate_transform2(c_current);
                x = T(1:3, 4);
                ...
                % Jacobian
                J = self.jacob0(c_current);    
                Ja = self.jacoba(J, x);
    
                % Damped least squares
                deltaQ = Ja'* pinv(Ja*Ja' + lambda^2 * eye(3)) * e;
    ...
    function config_vec = forward_actuation_to_config(self, rho, theta)
        lls = self.get_links(rho);          
    
        [phi, kappa] = self.calculate_phi_and_kappa(theta, rho);  
    ...
    function J = central_diff_jacobian(self, f_handle, q_current, h)
    ...
    function J = jacob0(self, c)
        ...
        for j = 1:n
            ...
            % single-link Jacobian 
            Jj = self.jacobianSingleLink(k, phi, l);   
        
            % adjoint of current transform
            AdT = self.adjoint(T);
        
            % map into space frame
            J(:, 3*j-2:3*j) = AdT * Jj;
    
            % propagate transform
            %T = T * arckinematicsone(k, l, phi);
            %[T_x, ~] = arckinematics([k, phi, l]);
            T_x = self.arckinematics(k, phi, l);
            T = T * T_x;
        end
    
end

    function J = jacobianSingleLink(self, k,phi,l)
        % YOUR CODE HERE
        if abs(k) < 1e-4
            % Linear velocity (v) limits as k -> 0
            v = [0, 0, 0; 
                 0, 0, 0; 
                 0, 0, 1];
            % Angular velocity (w) limits as k -> 0
            w = [0, 0, 0; 
                 0, 0, 0; 
                 0, 1, 0];
        else
            v = [(cos(phi)*(cos(k*l) - 1))/k^2, 0, 0;
                (sin(phi)*(cos(k*l) - 1))/k^2, 0, 0;
                -(sin(k*l) - k*l)/k^2, 0, 1];
            w = [-l*sin(phi), 0, -k*sin(phi);
    ...
    %SKEW - creates a skew symmetric matrix given a rotational orientation
    %vector
    
        s = [0 -w(3) w(2); w(3) 0 -w(1); -w(2) w(1) 0];
    end
    
    function J_a = jacoba(self, J_s, s)    
        J_a = -skew(s)*J_s(1:3,:) + J_s(4:6,:);
    end
